...
<a href="{{ $href }}"
   class="flex items-center gap-3 px-4 py-2.5 rounded-xl text-sm font-bold transition-all group
          {{ $active
             ? 'bg-primary-variant/10 text-primary-variant border border-primary-variant/20'
             : 'text-zinc-400 hover:text-white hover:bg-zinc-800/50 border border-transparent' }}">
    <svg class="w-5 h-5 shrink-0 {{ $active ? 'text-primary-variant' : 'text-zinc-500 group-hover:text-zinc-300' }}"
